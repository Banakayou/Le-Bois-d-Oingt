...
{
	public  RoadSystem.ObjectFrequency    m_assetFrequency      = RoadSystem.ObjectFrequency.HIGH;

	public  RoadNode                      m_currentNode         = null;                 // the current node that the pedestrian object will travel to 
    ...
    private RoadNode[] m_prevPedestrianNodes;
	...
	IEnumerator Start () 
	{
		if(RoadSystem.Instance)
		{
		    m_speedVariation    = Random.Range(0.0f, RoadSystem.Instance.m_globalSpeedVariation);
			m_lanePosXVariation = Random.Range(-RoadSystem.Instance.m_globalLanePosVariation, RoadSystem.Instance.m_globalLanePosVariation);
			m_lanePosZVariation = Random.Range(-RoadSystem.Instance.m_globalLanePosVariation, RoadSystem.Instance.m_globalLanePosVariation);
		}
	...
	{
		if(RoadSystem.Instance)
			RoadSystem.Instance.UnRegisterObject( this );
	}

    public void Spawn(Vector3 a_pos, RoadNode a_startNode)
	{
	...
	public bool HasVisitedNode( RoadNode a_node )
	{
		for(int nIndex = 0; nIndex < m_prevPedestrianNodes.Length; nIndex++)
		{
